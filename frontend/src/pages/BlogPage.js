      ...
      <div style={{ background: COLORS.cream, minHeight: "100vh", paddingTop: 100 }}>
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 24px" }}>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            <div style={{ background: selected.color, borderRadius: 20, padding: "40px 40px 32px", marginBottom: 32 }}>
              <div style={{ fontSize: 72, marginBottom: 12 }}>{selected.img}</div>
              <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                ...
                <span style={{ color: COLORS.slate, fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>{selected.date} · {selected.read} read</span>
              </div>
              <h1 style={{ fontSize: 38, fontWeight: 900, color: COLORS.navy, fontFamily: "'Playfair Display', Georgia, serif", margin: 0 }}>{selected.title}</h1>
            </div>
            <div style={{ background: COLORS.white, borderRadius: 20, padding: 40 }}>
              <p style={{ fontSize: 18, color: COLORS.slate, lineHeight: 1.8, fontFamily: "'DM Sans', sans-serif", marginBottom: 24, fontStyle: "italic" }}>{selected.excerpt}</p>
              {[
                ...
                <p key={i} style={{ fontSize: 16, color: COLORS.slate, lineHeight: 1.8, fontFamily: "'DM Sans', sans-serif", marginBottom: 20 }}>{para}</p>
              ))}
              <div style={{ borderTop: "1px solid #F0F0F0", paddingTop: 24, marginTop: 32, textAlign: "center" }}>
                <p style={{ fontWeight: 700, color: COLORS.navy, fontFamily: "'DM Sans', sans-serif", marginBottom: 16 }}>Need professional nursing care at home?</p>
    ...
    <div style={{ background: COLORS.cream, minHeight: "100vh", paddingTop: 100 }}>
      ...
      <div style={{ maxWidth: 1100, margin: "-32px auto 0", padding: "0 24px 96px" }}>
        <motion.div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }} initial="hidden" animate="show" variants={stagger}>
          {POSTS.map(post => (
            <motion.div key={post.id} variants={fadeUp} whileHover={{ y: -6, boxShadow: "0 20px 50px rgba(0,0,0,0.1)" }}
              onClick={() => setSelected(post)} style={{ background: COLORS.white, borderRadius: 20, overflow: "hidden", cursor: "pointer", border: "1px solid rgba(0,0,0,0.05)" }}>
              <div style={{ background: post.color, padding: "32px", textAlign: "center" }}>
                <div style={{ fontSize: 64 }}>{post.img}</div>
              </div>
              <div style={{ padding: 24 }}>
                <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
                  <span style={{ background: "#E8F6FF", color: COLORS.teal, fontSize: 11, fontWeight: 700, borderRadius: 20, padding: "3px 10px", fontFamily: "'DM Sans', sans-serif" }}>{post.category}</span>
                  <span style={{ fontSize: 12, color: COLORS.slate, fontFamily: "'DM Sans', sans-serif" }}>{post.read} read</span>
                </div>
                <h3 style={{ fontSize: 17, fontWeight: 800, color: COLORS.navy, margin: "0 0 10px", fontFamily: "'DM Sans', sans-serif", lineHeight: 1.4 }}>{post.title}</h3>
                <p style={{ fontSize: 13, color: COLORS.slate, lineHeight: 1.6, margin: "0 0 16px", fontFamily: "'DM Sans', sans-serif" }}>{post.excerpt.substring(0, 100)}...</p>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 12, color: COLORS.slate, fontFamily: "'DM Sans', sans-serif" }}>{post.date}</span>
