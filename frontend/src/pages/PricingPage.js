  ...
  return (
    <div style={{ background: COLORS.cream, minHeight: "100vh", paddingTop: 100 }}>
      ...
      <div style={{ maxWidth: 1100, margin: "-32px auto 0", padding: "0 24px 64px" }}>
        <motion.div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24, alignItems: "stretch" }} initial="hidden" animate="show" variants={stagger}>
          {pricing.map(plan => (
            <motion.div key={plan.id} variants={fadeUp} whileHover={{ y: -6 }}
              ...
              <h3 style={{ fontSize: 22, fontWeight: 800, color: plan.highlight ? COLORS.white : COLORS.navy, margin: "0 0 8px", fontFamily: "'DM Sans', sans-serif" }}>{plan.name}</h3>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 32 }}>
                {plan.features.map(f => (
                  <div key={f} style={{ display: "flex", alignItems: "center", gap: 10, fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: plan.highlight ? "rgba(255,255,255,0.8)" : COLORS.slate }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          style={{ background: COLORS.white, borderRadius: 20, padding: 32, marginTop: 40, textAlign: "center" }}>
          <h3 style={{ fontSize: 24, fontWeight: 800, color: COLORS.navy, fontFamily: "'DM Sans', sans-serif", margin: "0 0 8px" }}>Need a custom plan?</h3>
          <p style={{ color: COLORS.slate, fontFamily: "'DM Sans', sans-serif", marginBottom: 20 }}>We offer customized plans for long-term care, corporate wellness, and hospital partnerships.</p>
