        ...
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            <input type="text" placeholder="🔍  Search for answers..." value={search} onChange={e => setSearch(e.target.value)}
              style={{ width: "100%", maxWidth: 500, padding: "18px 24px", borderRadius: 16, border: "none", fontSize: 16, fontFamily: "'DM Sans', sans-serif", outline: "none", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }} />
          </motion.div>
        </div>
      </div>

      <div style={{ maxWidth: 1100, margin: "-32px auto 0", padding: "0 clamp(16px,4vw,24px) 96px" }}>
        <motion.div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 56 }} initial="hidden" animate="show" variants={stagger}>
          {CATEGORIES.map(cat => (
            <motion.div key={cat.title} variants={fadeUp} whileHover={{ y: -4, boxShadow: theme.shadowLg }}
              style={{ background: theme.bgCard, borderRadius: 16, padding: 24, display: "flex", alignItems: "center", gap: 16, cursor: "pointer", border: `1px solid ${theme.border}`, transition: "background 0.3s" }}>
              <div style={{ fontSize: 36 }}>{cat.icon}</div>
              <div>
                <h4 style={{ fontSize: 16, fontWeight: 800, color: theme.text, margin: "0 0 4px", fontFamily: "'DM Sans', sans-serif" }}>{cat.title}</h4>
                <p style={{ fontSize: 13, color: theme.textMuted, margin: 0, fontFamily: "'DM Sans', sans-serif" }}>{cat.count} articles</p>
              </div>
            </motion.div>
          ))}
        </motion.div>

        <h2 style={{ fontSize: "clamp(22px,4vw,32px)", fontWeight: 900, color: theme.text, fontFamily: "'Playfair Display', Georgia, serif", margin: "0 0 24px" }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ background: theme.bgCard, borderRadius: 16, padding: 40, textAlign: "center", border: `1px solid ${theme.border}` }}>
            ...
            <p style={{ color: theme.textMuted, fontFamily: "'DM Sans', sans-serif" }}>No results found. Try a different search or <span style={{ color: COLORS.teal, fontWeight: 700, cursor: "pointer" }} onClick={() => navigate("/contact")}>contact our team</span>.</p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {filtered.map((faq, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div style={{ padding: "20px clamp(16px,3vw,24px)", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", gap: 12 }} onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                  <h4 style={{ fontSize: 16, fontWeight: 800, color: theme.text, margin: 0, fontFamily: "'DM Sans', sans-serif" }}>{faq.q}</h4>
                  <motion.span animate={{ rotate: openFaq === i ? 180 : 0 }} style={{ color: COLORS.teal, fontSize: 18, flexShrink: 0, marginLeft: 8 }}>▼</motion.span>
                </div>
                <AnimatePresence>
                  {openFaq === i && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                      style={{ overflow: "hidden", borderTop: `1px solid ${theme.border}` }}>
                      <p style={{ padding: "16px clamp(16px,3vw,24px)", color: theme.textMuted, lineHeight: 1.7, fontFamily: "'DM Sans', sans-serif", fontSize: 15, margin: 0 }}>{faq.a}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          style={{ background: COLORS.navy, borderRadius: 20, padding: "clamp(28px,5vw,40px)", marginTop: 48, textAlign: "center" }}>
          <h3 style={{ fontSize: "clamp(20px,3vw,26px)", fontWeight: 900, color: COLORS.white, fontFamily: "'Playfair Display', Georgia, serif", margin: "0 0 12px" }}>Still need help?</h3>
