...
export default function TermsPage() {
  ...
  return (
    <div style={{ background: theme.bg, minHeight: "100vh", paddingTop: 100, transition: "background 0.3s" }}>
      <div style={{ background: COLORS.navy, padding: "clamp(48px,8vw,80px) clamp(20px,5vw,48px) clamp(60px,10vw,96px)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
            style={{ fontSize: 16, color: "rgba(255,255,255,0.5)", fontFamily: "'DM Sans', sans-serif" }}>Last updated: February 2024</motion.p>
      ...
      <div style={{ maxWidth: 860, margin: "-32px auto 0", padding: "0 clamp(16px,4vw,24px) 96px" }}>
        {SECTIONS.map((s, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
            style={{ background: theme.bgCard, borderRadius: 16, padding: "clamp(20px,3vw,28px) clamp(20px,3vw,32px)", marginBottom: 16, boxShadow: theme.shadow, border: `1px solid ${theme.border}`, transition: "background 0.3s" }}>
            <h3 style={{ fontSize: 20, fontWeight: 800, color: theme.text, margin: "0 0 12px", fontFamily: "'DM Sans', sans-serif" }}>{s.title}</h3>
            <p style={{ fontSize: 15, color: theme.textMuted, lineHeight: 1.8, margin: 0, fontFamily: "'DM Sans', sans-serif" }}>{s.content}</p>
          </motion.div>
        ))}
      </div>
    </div>
